...
function takePort(ep) {
    ...
    if (ep.localPort) {
        allocatedPorts[ep.localPort] = ep;
        return ep
    }
}
...
function allocate(desired) {
    ...
    if (desired) {
        ...
        var port = _.random(0xC000, 0xffff);
        // if busy, start searching for free
        if (port in allocatedPorts) {
            port = 0xC000;
            while (port in allocatedPorts) {
                port++;
                if (port > 0xffff) {
                    return
                }
