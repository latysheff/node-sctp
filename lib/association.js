...
class Association extends EventEmitter {
  constructor(endpoint, options, initChunk, callback) {
    ...
    this.localPort = this.endpoint.localPort
    ...
    this.on('data', (chunk, source) => {
        ...
        if (timeout) {
          if (this._sackTimeout) {
            ...
            this.log('trace', 'SACK set timeout', timeout)
            this._sackTimeout = setTimeout(
              () => {
                delete this._sackTimeout
                if (timeout) {
                  this.log('trace', 'SACK timed out after', timeout)
                }
                this._sack()
              }, timeout)
          }
    ...
    this.on('sack', (chunk) => {
      ...
      if (chunk.sack_info && chunk.sack_info.gap_blocks && chunk.sack_info.gap_blocks.length) {
        ...
        let absent = []
        let tmp = []
        _.each(chunk.sack_info.gap_blocks, (block, index) => {
          absent.push({
    ...
    this.on('abort', (chunk) => {
      ...
      this.bundleQueue = []
      //let user_error = _.find(chunk.error_causes, {cause: 'USER_INITIATED_ABORT'})
  ...
  _sendChunk(chunkType, options, destination, callback) {
    ...
    */
    if (chunkType === 'data' || chunkType === 'sack' || chunkType === 'heartbeat_ack') {
      ...
      this.bundling++
      // this._bundleTimout = setTimeout(this._bundle.bind(this), 0)
      setTimeout(this._bundle.bind(this), 0)
    } else {
      // no bundle
      setTimeout(() => {
  ...
  _sack() {
    this.sacks--
    if (this.sacks > 0) {
      // guard from already running procedure
      this.log('trace', this.sacks, 'sacks in queue - ignore')
      return
    }
    let gap_blocks = []
  ...
  _expireT3() {
    ...
    if (_.keys(this.sentChunks).length === 0) {
  ...
  _destroy() {
    ...
    if (this.endpoint) {
      for (let address in this.destinations) {
        delete this.endpoint.associations[address + ':' + this.remotePort]
      }
    }
  ...
  SEND(buffer, sctp_sndrcvinfo, callback) {
    ...
    if (error) {
       ...
       */
      if (this._sackTimeout) {
        this.log('trace', 'SACK cancel timer')
        clearTimeout(this._sackTimeout)
        delete this._sackTimeout
        this._sack()
      }
