...
class Association extends EventEmitter {
  constructor(endpoint, options, initChunk) {
    super()
    ...
    this.debugger = {}
    let label = `[${this.localPort}/${this.remoteAddress}:${this.remotePort}]`
    ...
    let key = this.remoteAddress + ':' + this.remotePort
    ...
    let PMTU = 1500 // todo
    if (!this.my_rwnd) this.my_rwnd = defs.net_sctp.RWND
    ...
    if (initChunk) {
      ...
      */
      ...
      // todo ssthresh?
      this.peer_tag = null
      this.my_tag = crypto.randomBytes(4).readUInt32BE(0)
      // COOKIE-WAIT, COOKIE-ECHOED, ESTABLISHED,
      // SHUTDOWN-PENDING, SHUTDOWN-SENT, SHUTDOWN-RECEIVED, SHUTDOWN-ACK-SENT

      this.peer_rwnd = null // TCB
      this.peer_last_tsn = null
      this.peer_max_tsn = null
      this.peer_min_tsn = null
    }

    this.my_next_tsn = SN(this.my_tag)
    this.mapping_array = []
    this.HTNA = this.my_next_tsn.copy()

    this.peer_ssn = []
    for (let sid = 0; sid < this.MIS; sid++) {
      this.peer_ssn.push(SN(0, 16))
    }
  ...
  onICMP(packet, code) {
    ...
    if (packet.v_tag && packet.v_tag !== this.peer_tag) return
    if (code === 4) {
  ...
  onData(chunk, source) {
    ...
    if (this.my_rwnd <= 0 && TSN.gt(this.peer_max_tsn)) {
      ...
      let isLast = TSN.gt(this.peer_max_tsn)
      if (isLast) {
        this.peer_max_tsn = TSN
      }
      let offset = TSN.delta(this.peer_min_tsn)
      if (offset <= 0 || this.mapping_array[offset - 1]) {
        ...
        if (TSN.gt(this.peer_last_tsn)) this._updateCumulative()
        if (chunk.packet) this.packetsSinceLastSack++
      }
    }
    let haveGaps = this.peer_last_tsn.lt(this.peer_max_tsn)
  ...
  onSack(chunk) {
    ...
    this.debugger.trace(
      '< sack c_tsn %d, peer_rwnd %d',
      chunk.c_tsn_ack,
      chunk.a_rwnd
    )
    this.peer_rwnd = chunk.a_rwnd

    let c_tsn_ack = SN(chunk.c_tsn_ack)
    let ackAdvanced = this.c_tsn_ack ? c_tsn_ack.gt(this.c_tsn_ack) : true
    this.c_tsn_ack = c_tsn_ack.copy()

    if (this.fastRecovery && c_tsn_ack.ge(this.fastRecoveryExitPoint)) {
      this.fastRecovery = false
      this.fastRecoveryExitPoint = null
    }
    let flightsize = this.flightsize
    for (let tsn in this.sent) {
      let TSN = SN(tsn)
    ...
    if (
      ...
      chunk.sack_info.gap_blocks.length
    ) {
      let gap_blocks = chunk.sack_info.gap_blocks
      this.debugger.trace('< gap_blocks ', chunk.c_tsn_ack, gap_blocks)
      /*
      ...
      let absent = []
      gap_blocks.forEach((block, idx) => {
        // todo rewrite with SN api
        absent.push({
          start: SN(
            idx
              ? chunk.c_tsn_ack + gap_blocks[idx - 1].finish + 1
              : chunk.c_tsn_ack + 1
          ),
          finish: SN(chunk.c_tsn_ack + block.start - 1),
        })
        for (
      ...
      if (this.my_next_tsn.eq(this.c_tsn_ack.copy().inc(1))) {
        /*
    ...
    if (
      ...
      chunk.sack_info.duplicate_tsn.length
    ) {
      this.debugger.trace(
    ...
    if (ackAdvanced && this.flightsize) {
      ...
      if (
        ...
        this.debugger.trace(
          'increase cwnd by %d to %d, ssthresh %d',
          cwndIncrease,
          this.cwnd,
          this.ssthresh
        )
  ...
  onInitAck(chunk, source) {
    if (this.state === 'COOKIE-WAIT') {
      this.debugger.info('< init_ack cookie', chunk.state_cookie)
      ...
      this._sendChunk(
        ...
        source,
        () => {
          this.debugger.info('sent cookie_echo', chunk.state_cookie)
        }
      )
      /*
  ...
  onError(chunk) {
    ...
    if (
      chunk.error_causes.some(
        error_cause => error_cause.cause === 'STALE_COOKIE_ERROR'
      )
    ) {
  ...
  _scan() {
    let max = this.peer_max_tsn.delta(this.peer_min_tsn) - 1
    let res = null
    this.debugger.trace(
    ...
    for (let i = 0; i <= max; i++) {
      ...
      if (typeof chunk === 'object') {
        if (chunk.flags.B) {
          ...
          if (
            chunk.flags.U ||
            SN(chunk.stream_sn).eq(this.peer_ssn[chunk.stream_id])
          ) {
            if (!chunk.flags.E) {
              this.debugger.trace(
  ...
  _updateTrack() {
    let offsetTracking
    let max = this.peer_max_tsn.delta(this.peer_min_tsn)
    for (let i = 0; i < max; i++) {
  ...
  _sack() {
    ...
    let gap_blocks = []
    let max = this.peer_max_tsn.delta(this.peer_last_tsn)
    let offset = this.peer_last_tsn.delta(this.peer_min_tsn)
    let start
    let gap
    for (let i = 0; i <= max; i++) {
  ...
  _deliver(user_data, stream) {
    this.debugger.debug('< receive user data %d bytes', user_data.length)
    this.my_rwnd += user_data.length
  ...
  _bundle() {
    ...
    if (sack) processedQueue.unshift(sack)
    ...
    processedQueue.forEach((chunk, index) => {
      ...
      let finish = index === processedQueue.length - 1
      let full = bundledLength + chunk.size > mtu
  ...
  _destroy() {
    ...
    if (this.endpoint) {
      ...
      let index = this.endpoint.associations.indexOf(this)
  ...
  ABORT(reason) {
    ...
    let error_cause = { cause: 'USER_INITIATED_ABORT' }
    if (reason) error_cause.abort_reason = reason
    this._abort({ error_causes: [error_cause] })
  }
  ...
  SEND(buffer, sctp_sndrcvinfo, callback) {
    ...
    this.debugger.trace('SEND %d bytes, %o', buffer.length, sctp_sndrcvinfo)
    let error = false
    if (
      this.state === 'SHUTDOWN-PENDING' ||
      this.state === 'SHUTDOWN-RECEIVED'
    ...
    if (error) {
      ...
      if (this.flightsize + defs.net_sctp.max_burst * this.PMTU < this.cwnd) {
        ...
        this.debugger.trace(
          'adjust cwnd to flightsize + Max.Burst*MTU = %d',
          this.cwnd
        )
      }
      let chunk
      ...
      if (!this.SSN[stream]) this.SSN[stream] = SN(0, 16)
  ...
  _updatePeer(chunk) {
    ...
    this.peer_last_tsn = SN(this.peerInitialTSN).prev()
