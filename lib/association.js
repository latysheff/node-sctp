...
class Association extends EventEmitter {
  ...
  onICMP(packet, code) {
    ...
    if (code === 4) {
      ...
      this.debugger.warn('< ICMP unreachable/unknown/prohibited')
  ...
  onData(chunk, source) {
    ...
    const result = this.reassembly.process(chunk)
  ...
  onSack(chunk) {
    ...
    if (ackAdvanced && this.flightsize) {
      ...
      if (this.cwnd <= this.ssthresh && this.cwnd <= this.flightsize && !this.fastRecovery) {
        ...
        this.debugger.warn('increase cwnd +%d from %d to %d, ssthresh %d',
