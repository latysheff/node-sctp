...
class Association extends EventEmitter {
  constructor(endpoint, options, initChunk) {
    ...
    if (this.logger && (typeof this.logger.log === 'function')) {
      this.log = (level, ...rest) => {
        this.logger.log(level, 'association - [', this.localPort, '<->', this.remotePort, this.remoteAddress, ']', ...rest)
      }
    } else {
      this.log = () => {
      }
    ...
    this.log('debug', 'create association')

    // todo provide also way to iterate
    endpoint.associations_lookup[this.remoteAddress + ':' + this.remotePort] = this
    ...
    this.on('data', (chunk, source) => {
        ...
        if (this.packetsSinceLastSack >= this.sack_freq || !this.everSentSack || haveGaps || isDuplicate || zeroRwndDrop) {
          ...
          this.sacks++
          // todo this postponed send may result in sack chunk being placed arriving after data chunk
          // this results in protocol violation - 'DATA chunk followed by chunk of type 03'
          // setTimeout(() => {
          this._sack()
          // }, 0)
        } else {
          if (this._sackTimeout) {
            ...
            this.log('trace', 'SACK timeout set', this.sack_timeout)
            this._sackTimeout = setTimeout(
              () => {
                this.log('trace', 'SACK timeout expired', this.sack_timeout)
                this._sack()
              }, this.sack_timeout)
          }
        }
    ...
    this.on('abort', (chunk) => {
      ...
      if (this.bundleQueue.length) {
        this.log('trace', 'abandon sending of chunks', this.bundleQueue.length)
      }
      this.bundleQueue = []
  ...
  _sendChunk(chunkType, options, destination, callback) {
    ...
    this.log('debug', '> send chunk', chunkType)
    this.log('trace', '> ', chunk)
    /*
  ...
  _deliver(user_data, stream) {
    ...
    if (this.listeners('DATA ARRIVE')) {
      this.log('trace', 'EMIT DATA ARRIVE', this.myRwnd)
  ...
  _bundle() {
    ...
    this.log('trace', 'process bundle queue', processedQueue)
  ...
  ABORT(reason) {
    ...
    this._down()
    // If the association is aborted on request of the upper layer, a User-Initiated Abort error cause
    ...
    if (chunk.ipv4_address) {
      chunk.ipv4_address.forEach((address) => {
          ...
          if (!(address in this.destinations)) {
            ...
            this.endpoint.associations_lookup[address + ':' + this.remotePort] = this
          }
        }
      )
    }
