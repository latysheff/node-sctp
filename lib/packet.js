...
function Chunk(chunkType, options) {
    ...
    this.chunkId = chunks[chunkType].id;

    if (typeof options !== 'object') {
        options = {}
    }
    this.flags = options.flags;
    var params = chunks[chunkType].params || {};
    var chunk = this;
...
Chunk.prototype.fromBuffer = function (buffer) {
    ...
    offset += 4;
    if (chunks[this.chunkId]) {
        this.chunkType = chunks[this.chunkId].chunkType;
        params = chunks[this.chunkType].params || {}
    } else {
        this.chunkType = 'unknown';
        return
    }
    if (chunks[this.chunkType].flags_filter) {
        this.flags = chunks[this.chunkType].flags_filter.decode.call(this, this.flags)
    }
...
Chunk.prototype.toBuffer = function () {
    ...
    if (chunks[this.chunkType].flags_filter) {
        this.flags = chunks[this.chunkType].flags_filter.encode.call(this, this.flags)
    } else {
        this.flags = 0
    }
    this._filter('encode');
    var params = chunks[this.chunkType].params || {};
    var length;
    var key;
    var padding;
    var chunk = this;
    _.each(chunk, function (value, key) {
        if (params[key]) {
            ...
            _.each(values, function (value) {
                ...
                padding = length % 4;
                if (padding) chunk.message_length += (4 - padding)
            });
        }
    });
