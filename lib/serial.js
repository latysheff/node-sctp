...
class SerialNumber {
    ...
    delta(that) {
        var result = this.number - that.number;
    ...
    prev() {
        return new SerialNumber(this.number === 0 ? this._max : this.number - 1, this.serialBits);
    }

    next() {
        return new SerialNumber(this.number === this._max ? 0 : this.number + 1, this.serialBits);
    }
}
