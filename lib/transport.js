...
class RawTransport extends Transport {
  constructor(options) {
    ...
    rawsocket.on('message', (buffer, src) => {
      ...
      let dst = ip.toString(buffer, 16, 4)
  ...
  sendPacket(src, dst, packet, callback) {
    ...
    if (src) {
      ...
      let headerBuffer = createHeader({ src, dst, payload })
  ...
  processICMPPayload(buffer, code) {
    ...
    let dst = ip.toString(buffer, 16, 4)
    let src = ip.toString(buffer, 12, 4)

    let sctpBuffer = buffer.slice(headerLength)
    this.debug('< ICMP code %d', code, src, dst, sctpBuffer)

    let packet = Packet.fromBuffer(sctpBuffer)
...
function createHeader(packet) {
  let buffer = Buffer.from(IP_HEADER_TEMPLATE)
