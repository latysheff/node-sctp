...
class RawTransport extends Transport {
  constructor() {
    ...
    process.nextTick(this.enableDiscardService.bind(this))
    process.nextTick(this.enableICMP.bind(this))
  }
  ...
  enableICMP() {
    ...
    */
    ...
    this.icmpsocket.on('message', (buffer, src) => {
      ...
      this.debug('< ICMP from %s (%d bytes)', src, buffer.length, buffer)
      if (buffer.length < 42) {
  ...
  processICMPPacket(buffer) {
    this.debug('< process ICMPPacket (%d bytes)', buffer.length, buffer)
    /*
