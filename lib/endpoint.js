...
class Endpoint extends EventEmitter {
  constructor(options) {
    ...
    this.on('packet', (packet, src, dst) => {
      ...
      packet.chunks.every((buffer, index) => {
        ...
        if (chunk.chunkType === 'abort') {
          ...
          */
          abortPacket = true
          const association = this._getAssociation(src, packet.src_port)
          if (association) {
            /*
    ...
    this.on('init', (chunk, src, dst, header, association) => {
      ...
      const myTag = ~~(Math.random() * 0x7FFFFFFF)
      const cookie = this.createCookie(chunk.buffer, header, myTag)
    ...
    this.on('cookie_echo', (chunk, src, dst, header, association) => {
      ...
      */
      ...
      if (cookieData) {
        ...
        const initChunk = Chunk.fromBuffer(cookieData.init)
        const options = {
          remoteAddress: src,
        ...
        if (association) {
          ...
          this.debugger.debug('association my_tag %d peer_tag %d, cookie my_tag %d peer_tag %d',
            association.my_tag, association.peer_tag, cookieData.my_tag, initChunk.initiate_tag)
          let action = ''
          if (association.my_tag === cookieData.my_tag) {
            // B or D
            if (association.peer_tag === initChunk.initiate_tag) {
              action = 'D'
            } else {
              action = 'B'
            }
          } else if (association.peer_tag === initChunk.initiate_tag) {
          ...
          be running, and send a COOKIE ACK.
              */
              association.peer_tag = initChunk.initiate_tag
        ...
        const newAssociation = new Association(this, options, initChunk)
  ...
  createCookie(chunk, header, myTag) {
    ...
    const mac = hash.digest() // Length 16
  ...
  validateCookie(cookie, header) {
    ...
    const information = cookie.slice(16, 32)
    const init = cookie.slice(32)
    /*
  ...
  ASSOCIATE(options) {
    ...
    const association = new Association(this, {
  ...
  static INITIALIZE(options, callback) {
    ...
    const port = transport.register(endpoint)
    if (port) {
