...
class Endpoint extends EventEmitter {
  constructor(options) {
    ...
    this.debugger.info('creating endpoint %s:%d', this.localAddress, this.localPort)
  ...
  onPacket(packet, src, dst) {
    ...
    // This also happens if ABORT chunk exists
    ...
    if (decodedChunks.length > 1 &&
      ...
      // Nothing happens here for now, only debug
    }
    ...
    if (chunkTypes.abort) {
      if (
        (packet.v_tag === association.my_tag && !abortChunk.flags.T) ||
        (packet.v_tag === association.peer_tag && abortChunk.flags.T)
      ) {
        /*
    ...
    decodedChunks.forEach((chunk, index) => {
      chunk.dataPacket = index === lastDataChunk
