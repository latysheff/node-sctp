...
class Endpoint extends EventEmitter {
  constructor(options, logger) {
    ...
    this.log('trace', 'create endpoint', options)
    ...
    this.on('packet', (packet, source, destination) => {
      ...
      let tBit = false
      let chunkTypes = {}
      packet.chunks.forEach((buffer, index) => {
      ...
      switch (true) {
        ...
        case chunkTypes['abort']:
          if (association) {
             ...
             */
            if ((packet.verification_tag === association.myTag) && !tBit
              || (packet.verification_tag === association.peerTag) && tBit
            ) {
              // ok
            } else {
              this.log('warn', 'silently discard according to B) Rules for packet carrying ABORT', packet)
              this.log('warn', 'verification_tag', packet.verification_tag, 'tbit', tBit, 'myTag', association.myTag, 'peerTag', association.peerTag
              )
              return
            }
          } else {
  ...
  _sendPacket(host, port, tag, chunks, callback) {
    ...
    rawsocket.sendPacket(this.localActiveAddress, host, packet, callback)
  }
  ...
  _getAssociation(host, port) {
    this.log('trace', 'find association for', host, port)
