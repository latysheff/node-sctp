...
class Endpoint extends EventEmitter {
  constructor(options) {
    ...
    this.log('trace', 'creating endpoint', this.localPort, this.localAddress || '')
    ...
    this.on('icmp', (packet, source, destination, code) => {
      let association = this._getAssociation(destination, packet.dst_port)
      if (association) {
        this.log('warn', '< received ICMP reject for', destination, ':', packet.dst_port)
    ...
    this.on('packet', (packet, source, destination) => {
      if (!Array.isArray(packet.chunks)) {
        this.log('warn', '< received empty packet from', source, ':', packet.src_port)
        return
      }
      this.log('debug', '< received packet from', source, ':', packet.src_port)
      let emulateLoss
      if (emulateLoss) {
      ...
      packet.chunks.every((buffer, index) => {
        ...
        if (chunk.chunkType === 'abort') {
          /*
           ...
           ...
           DATA chunks MUST NOT be bundled with ABORT.  Control chunks (except for INIT, INIT ACK, and
           SHUTDOWN COMPLETE) MAY be bundled with an ABORT, but they MUST be
           placed before the ABORT in the SCTP packet or they will be ignored by the receiver.
          */
          abortPacket = true
          let association = this._getAssociation(source, packet.src_port)
          if (association) {
            /*
              The receiver of an ABORT MUST accept the packet if
                the Verification Tag field of the packet matches its own tag and the T bit is not set
                OR if it is set to its peer's tag and the T bit is set in the Chunk Flags.
              Otherwise, the receiver MUST silently discard the packet and take no further action.
             */
            if ((packet.verification_tag === association.myTag) && !chunk.flags.T || (packet.verification_tag === association.peerTag) && chunk.flags.T) {
              this.log('warn', 'emit ONLY abort for association')
              association.emit(chunk.chunkType, chunk, source, packet)
            } else {
              this.log('warn', 'silently discard according to B) Rules for packet carrying ABORT', packet)
              this.log('debug', 'verification_tag', packet.verification_tag, 'tbit', tBit, 'myTag', association.myTag, 'peerTag', association.peerTag
              )
            }
          } else {
            /*
              8.4.  Handle "Out of the Blue" Packets
              2)  If the OOTB packet contains an ABORT chunk, the receiver MUST silently discard the OOTB packet and take no further action.
             */
            this.log('trace', 'OOTB abort', packet)
          }
      ...
      switch (true) {
        ...
        case chunkTypes['cookie_echo']:
          // after processing (check for tags will happen there) this chunk association can be established
        ...
        default:
          if (association) {
            ...
            // we don't have chunks in the packet that can establish or close association - nothing to do
            this.log('warn', 'OOTB packet')
            return
          }
      }
      ...
      if (discardPacket) {
        ...
        if (errors.length > 0) {
          if (association) {
            ...
            this._sendPacket(source, packet.src_port, packet.verification_tag, [abort.toBuffer()])
          }
        }
        return
      }
      ...
      decodedChunks.forEach((chunk, index) => {
        ...
        this.log('debug', 'processing chunk', chunk.chunkType, 'from', source, ':', header.src_port)
        if (!association) association = this._getAssociation(source, header.src_port)
        if (association) {
          if (chunk.chunkType === 'init' || chunk.chunkType === 'cookie_echo') {
            this.emit(chunk.chunkType, chunk, source, destination, header, association)
          } else {
            this.log('debug', 'emit chunk', chunk.chunkType, 'for association')
            association.emit(chunk.chunkType, chunk, source, header, true)
          }
        } else {
    ...
    this.on('init', (chunk, source, destination, header, association) => {
      ...
      if (errors.length > 0) {
        ...
        this._sendPacket(source, header.src_port, chunk.initiate_tag, [abort.toBuffer()])
        return
      }
      let myTag = ~~(Math.random() * 0x7fffffff)
      ...
      this._sendPacket(source, header.src_port, chunk.initiate_tag, [initAck.toBuffer()])
      /*
  ...
  _sendPacket(host, port, tag, chunks, callback) {
    this.log('debug', '> send packet', this.localActiveAddress, '->', host, port, tag, chunks)
  ...
  validateCookie(cookie, header) {
    ...
    if (mac.equals(cookie.slice(0, 16))) {
      ...
      /*
       ...
       silently discarded.
       */
      if (header.src_port === result.src_port &&
  ...
  _getAssociation(host, port) {
    this.log('trace', 'trying to find association for', host, port)
  ...
  static INITIALIZE(options, callback) {
    ...
    let port = transport.register(endpoint) // <- this is synchronous for now, but could be async
    if (port) {
