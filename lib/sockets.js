...
class SCTPStreamWritable extends Writable {
  ...
  _write (chunk, encoding, callback) {
    ...
    this.socket.customWrite(chunk, options, callback)
  }
}
...
class Socket extends Duplex {
  ...
  _write (chunk, encoding, callback) {
    ...
    this.customWrite(chunk, options, callback)
  }

  customWrite (chunk, options, callback) {
    const writeCount = this.writeCount++
    this.debugger.info('> write #%d to socket [%d bytes]', writeCount, chunk.length)
    if (!this.association) {
      return callback(new Error('no association established'))
    }

    if (Number.isInteger(chunk.ppid)) {
      options.ppid = chunk.ppid
    }
    ...
    let offset = 0
    let size = this.association.peer_rwnd / 4
    let final = false
    ...
    const sendPart = () => {
      ...
      this.association.SEND(chunk.slice(offset, offset + size), options, (error) => {
        this.debugger.debug('> write socket #%d complete [%d bytes] offset %d', writeCount, size, offset)
        if (error) {
