...
class Socket extends Duplex {
  ...
  _write(chunk, encoding, callback) {
    this.debugger.debug('> write socket %d bytes', chunk.length)
    if (!this.association) {
      return callback(new Error('no association established'))
    }
    const options = {}
    options.stream_id = this.stream_id
    this.bytesWritten += chunk.length
    return this.association.SEND(chunk, options, callback)
  }
    ...
    association.on('DATA ARRIVE', streamId => {
      ...
      if (this.listenerCount('stream') > 0) {
        if (!this.streamsReadable[streamId]) {
          this.streamsReadable[streamId] = new SCTPStreamReadable()
...
class Server extends EventEmitter {
  ...
  _listen(options, callback) {
    options = options || {}
    this.debugger.info('server listen %o', options)
