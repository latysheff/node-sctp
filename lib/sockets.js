...
class Socket extends Duplex {
  ...
  bufferedCustomWrite (chunk, options, callback) {
    if (this._writeLock) {
      this._writeQueue.push({ chunk, options, callback })
    } else {
      this._writeLock = true
      this.customWrite(chunk, options, () => {
        this._writeLock = false
        callback()
        if (this._writeQueue.length) {
          const { chunk, options, callback } = this._writeQueue.shift()
          this.bufferedCustomWrite(chunk, options, callback)
        }
      })
    }
  ...
  customWrite (chunk, options, callback) {
    ...
    // TODO split in Association

    let offset = 0
    let size = Math.max(this.association.PMTU, this.association.peer_rwnd) - 52
    let final = false
    ...
    const sendPart = () => {
      ...
      this.association.SEND(chunk.slice(offset, offset + size), options, (error) => {
        this.debugger.debug('> write #%d complete, stream %d [%d bytes] offset %d', writeCount, options.stream_id, size, offset)
        if (error) {
