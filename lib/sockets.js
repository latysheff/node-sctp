...
class Socket extends Duplex {
  ...
  _write(chunk, options, callback) {
    this.debugger.debug('> write %o, %d bytes', this.sctp_sndrcvinfo, chunk.length)
    if (this.association) {
      this.bytesWritten += chunk.length
      return this.association.SEND(chunk, this.sctp_sndrcvinfo, callback)
    }
    // Todo
  ...
  establish(endpoint, association) {
    ...
    association.on('DATA ARRIVE', stream => {
      ...
      if (buffer) {
        this.debugger.debug('< DATA ARRIVE %d bytes on stream %d', buffer.length, stream)
